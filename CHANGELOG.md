...
## [1.0.8] - 2026-09-11
### Ditambahkan (Added)
- **Pemindai & Filter NIK Cepat (REST API)**:
  - Modul multithreading `fast_filter_service.py` untuk memindai 100–1000 NIK dalam waktu 15–30 detik (150–300 ms/NIK).
  - Ekstraksi langsung kategori: *Rumah Tangga*, *Usaha Mikro*, atau *Belum Terdaftar*.
  - Pembuatan otomatis 2 file Excel: `hasil_filter_nik.xlsx` (laporan audit detail) dan `data_siap_proses.xlsx` (hanya NIK valid).
  - Tombol GUI baru: ⚡ Scan NIK Cepat (REST) lengkap dengan live counter dan tombol cepat untuk langsung memakai data bersih ke bot.
- **Pemeriksa Legalitas NIB Usaha Mikro (OSS)**:
  ...
  - Validasi otomatis jenis usaha terhadap Surat Edaran Dirjen Migas No. B-2461/MG.05/DJM/2022 (deteksi larangan untuk restoran besar, hotel, binatu/laundry komersil, peternakan besar, dan manufaktur).
  - Tombol GUI baru: 🏢 Cek NIB Usaha Mikro (OSS) untuk cek legalitas perizinan dan kelayakan subsidi 3 kg.
- **Pemisahan Pintar Tabung UM vs RT (Smart Tabung Split)**:
  ...
  - Penyesuaian otomatis tombol + pada UI transaksi Pertamina dan pemotongan kuota lisensi sesuai jumlah tabung aktual.
...
- **Pembaruan Tempat & Tanggal Lahir (TTL)**:
  ...
  - Alur klik tombol SELANJUTNYA (`[data-testid='btnSubmitUpdate']`), konfirmasi YA, PERBARUI DATA PELANGGAN, hingga kembali ke transaksi.
- **Kompilasi Kotlin Android APK**:
  - Memperbaiki dependensi `QueuePangkalanItem`, `canUseMultiPangkalan`, dan multi-pangkalan storage di `CredentialStore.kt`.
  - Android APK v1.0.7 terverifikasi sukses dikompilasi (`gradlew compileDebugKotlin` & `assembleDebug`).
...
- Multi-pangkalan queue runner eksklusif untuk paket Enterprise.
...
1. **Peningkatan Versi (Semantic Versioning)**:
   - **PATCH** (misal 1.0.8 -> 1.0.9): Untuk perbaikan bug, penyesuaian selector UI, dan perbaikan minor.
   - **MINOR** (misal 1.0.8 -> 1.1.0): Untuk penambahan fitur atau menu baru yang kompatibel ke belakang.
   - **MAJOR** (misal 1.0.8 -> 2.0.0): Untuk perombakan arsitektur besar atau perubahan protokol API yang memutus alur lama.
2. **Setiap Rilis Baru Wajib**:
   ...
   - Membuat Git Tag untuk rilis tersebut (misal: `git tag v1.0.8`).
